...
export  var systemName = "SYSTEM_CARGO_AUX_PREPROC"
	...
	if ship.getConfig(slot) != systemName:
		...
		for node in ship.get_children():
			if node.slot == "cargo.equipment":
				...
				#print("Changed mineral efficiency of %s" % node.systemName, " to %f" % newMinEff)
